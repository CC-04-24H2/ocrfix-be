...
const getRefreshTokenHandler = async (req, res) => {
  ...
  const bodyError = authValidator.validateRefresh();
  if (bodyError.length !== 0) {
    const response = Response.defaultBadRequest(bodyError);
    return res.status(response.code).json(response);
  }
  ...
  try {
    ...
    decodedToken = jwt.verify(body.refresh_token, secret);
  } catch (error) {
    const response = Response.defaultUnauthorized(error.message);
    return res.status(response.code).json(response);
  }

  const user = await User.findByPk(decodedToken.id);
  if (!user) {
    const response = Response.defaultUnauthorized('invalid credentials');
    return res.status(response.code).json(response);
  }
  ...
  const response = Response.defaultOK('Token refreshed successfully', {
    access_token: accessToken,
    refresh_token: refreshToken,
  });
  return res.status(response.code).json(response);
...
module.exports = { registerHandler, loginHandler, getRefreshTokenHandler };
